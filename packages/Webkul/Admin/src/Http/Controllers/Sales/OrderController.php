    ...
    public function advanceStatus(int $id)
{
    ...
    $statuses = [
        \Webkul\Sales\Models\Order::CUSTOM_STATUS_TEDARIK,
        \Webkul\Sales\Models\Order::CUSTOM_STATUS_KUTULAMA,
        \Webkul\Sales\Models\Order::CUSTOM_STATUS_KARGODA,
        \Webkul\Sales\Models\Order::CUSTOM_STATUS_YOLDA,
        \Webkul\Sales\Models\Order::CUSTOM_STATUS_TESLIM,
    ];

    $currentStatus = $order->custom_status;
    $currentIndex = array_search($currentStatus, $statuses);

    if ($currentIndex !== false && $currentIndex < count($statuses) - 1) {
        $order->custom_status = $statuses[$currentIndex + 1];
        $order->save();

        session()->flash('success', 'Sipariş durumu başarıyla güncellendi.');
    } else {
        session()->flash('info', 'Sipariş zaten son aşamada.');
    }
